...
function getGeoLocation(query, limit =5) {     //put this as a function so we can call this anytime query is the location passed in the function, 5 is the limit of results
  return fetch(
    `http://api.openweathermap.org/geo/1.0/direct?q=${query}&limit=${limit}&appid=${API_key}`
  );
}

function getCurrentWeather(arguments) {   //arguments are lat and lon  where the retured value of the of lat and lon passed into the function
  return fetch(
